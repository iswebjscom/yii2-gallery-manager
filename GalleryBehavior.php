...
class GalleryBehavior extends Behavior
{
     ...
     * @return GalleryImage[]
     */
    public function getImages($offset=2,$limit=2)
    {
        if ($this->_images === null) {
            $query = new \yii\db\Query();
            $imagesData = $query
                ->select(['id', 'label', 'content', 'link','position'])
                ->from($this->tableName)
     ...
     * @return GalleryImage[]
     */
    public function getImagesMore($offset=2,$limit=2,$pid)
    {
            $query = new \yii\db\Query();
            $imagesData = $query
                ->select(['id', 'label', 'content', 'link','position'])
                ->from($this->tableName)
                ->where(['type' => $this->type ? $this->type : CmsBlockType::TYPE_ALBUM, 'block_id' => $pid])
                ->offset($offset)
                ->limit($limit)
                ->orderBy(['position' => 'asc'])
                ->all();
    ...
    public function getImagePageCount($limit=5,$pid)
    {
            $query = new \yii\db\Query();
            $imagesData = $query
                ->select(['id', 'label', 'content', 'link','position'])
        ...
        return ceil(count($imagesData)/$limit);
    }
